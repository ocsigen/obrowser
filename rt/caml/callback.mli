...
(** Registering Caml values with the runtime.
   ...
   under a symbolic name, so that JS code can later call back registered
   Caml functions, or raise registered Caml exceptions.
*)

val register : string -> 'a -> unit
(** [Callback.register n v] registers the value [v] under
   the name [n]. JS code can later retrieve a handle to [v]
...
(** [Callback.register_exception n exn] registers the
   ...
   under the name [n]. JS code can later retrieve a handle to
