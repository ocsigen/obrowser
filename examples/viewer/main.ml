...
(* ex: <a href="test.jpg" id="::viewer::test.jpg"><img src="thumb.jpg" /></a> *)
let pictures_dir = "pictures/" ;;
let thumbnails_dir = "thumbs/" ;;
...
let display_img idx =
  ...
  let vimg = img ~src:(pictures_dir ^ (!imgs).(!current)) () in
  let mask = 
    div ~style:"position: fixed; right: 0px; top: 0px; width: 100%; height: 100%;
              \ background-color: black; opacity: .8;" []
  and pdiv = 
    div ~style:"position: fixed; right: 10px; bottom: 10px; -moz-border-radius: 5px; border: 1px white solid ;
              \ padding: 10px; background-color: black; text-align: center;" [vimg ; br ()]
  in
      ...
      (span [a
	       ~onclick:(fun () ->
			   let rec play t =
			     ...
			     Node.set_attribute vimg "src" (pictures_dir ^ (!imgs).(!current)) ;
